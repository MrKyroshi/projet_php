...
if (!empty($_POST)){
    ...
    if ($resultat){//si il y a un retour de la requête on stop l'inscription
       $_SESSION['messages']['danger'][]='Un compte existe déjà à cette mail, veuillez procéder à une demande de réinitialisation de mot de passe';
       header('location:./inscription.php');
       exit();
    }else{

        if($_POST['mdp'] !== $_POST['confirmPassword']){
            $erreur.='Les mots de passe ne correcpondent pas ';
        }
        if (empty($erreur)){
